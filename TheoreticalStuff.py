...
class AVLTree(object):
	...
	def find_start_fingertree(self, key):
		...
		while (None != cur):
			if (cur.get_key() < key):
				return cur.get_right(), distance
			cur = cur.get_parent()
			distance += 1
		return self.root, distance - 1
	...
	def insert(self, key, val, use_finger = True):
		...
		fixes = 0
		if (use_finger):
			start, fixes = self.find_start_fingertree(key)
			cur, dist = self.create_node_bst(key, val, start)
			fixes += dist
		else:
			cur, _ = self.create_node_bst(key, val, self.root)
	...
	def split(self, node):
		...
		while (None != parent):
			...
			if (parent.get_right().get_key() == node.get_key()):
				...
				cost = left.join(temp, parent.get_key(), parent.get_value(), False)
			else:
				temp = AVLTree()
				temp.root = parent.get_right()
				temp.root.set_parent(None)
				cost = right.join(temp, parent.get_key(), parent.get_value(), False)
			sumCost += cost
			if (maxCost < cost):
				maxCost = cost
			joinCount += 1
		...
		while (temp.get_right().is_real_node()):
			temp = temp.get_right()
		left.maximum = temp
		return [left, right], sumCost / joinCount, maxCost
	...
	def join(self, tree, key, val, update_maximum = True):
		...
		if (goLeft):
			x.set_left(t1root)
			x.set_right(b)
			x.set_parent(b.get_parent())
			if (None == x.get_parent()):
				self.root = x
			else:
				x.get_parent().set_left(x)
			b.set_parent(x)
			t1root.set_parent(x)
...
def experiment(array):
	...
	for i in array:
		t1.insert(i, i)
		t2.insert(i, i)
	_, aveCost, maxCost = t1.split(t1.select(random.randint(1, len(array))))
	print("Random: " + str(aveCost) + ", " + str(maxCost))
	node = t2.get_root().get_left()
	while (node.get_right().is_real_node()):
		node = node.get_right()
	_, aveCost, maxCost = t2.split(node)
	print("deterministic: " + str(aveCost) + ", " + str(maxCost))

		

for i in range(10,11):
	print(i)
	arr = ArrayCreator.CreateArray(1, i)
	experiment(arr)
